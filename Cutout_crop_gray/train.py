...
def train(train_loader, epoch, model, optimizer, criterion):
    ...
    for i, (input, target) in enumerate(train_loader):
        ...
        if i % print_freq == 0:
            progress.print(i)
    f = open('/root/volume/MidProject/Cutout_crop_gray/'+'train_2'+'.txt','a')
    save_acc = str(epoch)+'\t'+str(top1.avg)+'\t'+str(top5.avg)+'\n'
    f.write(save_acc)
    f.close()
...
def test(test_loader,epoch, model):
    ...
    for i,(input,target) in enumerate(test_loader):
        ...
        top1.update(acc1[0].item(), input.size(0))
        top5.update(acc5[0].item(), input.size(0))
    g = open('/root/volume/MidProject/Cutout_crop_gray/'+'test_2'+'.txt','a')
    save_acc = str(epoch)+'\t'+str(top1.avg)+'\t'+str(top5.avg)+'\n'
    g.write(save_acc)
    g.close()
...
for epoch in range(epochs):
    ...
    if best_acc < test_acc:
        best_acc = test_acc
        torch.save(model.state_dict(), '/root/volume/MidProject/Cutout_crop_gray/model_best_2.pt')

torch.save(model.state_dict(),'/root/volume/MidProject/Cutout_crop_gray/model_latest_2.pt')
