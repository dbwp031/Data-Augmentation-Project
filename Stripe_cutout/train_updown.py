...
from utils import *
from cutout_updown import Cutout
...
filename = "stripe_updown_"+str(n_holes)+'.txt'
...
if dataset == 'cifar10':
    num_classes = 10
    train_dataset = datasets.CIFAR10(root='/root/volume/MidProject/data/',
                                     train=True,
                                     transform=train_transform,
                                     download=True)

    test_dataset = datasets.CIFAR10(root='/root/volume/MidProject/data/',
                                    train=False,
                                    transform=test_transform,
                                    download=True)
elif dataset == 'cifar100':
    num_classes = 100
    train_dataset = datasets.CIFAR100(root='/root/volume/MidProject/data/',
                                      train=True,
                                      transform=train_transform,
                                      download=True)

    test_dataset = datasets.CIFAR100(root='/root/volume/MidProject/data/',
                                     train=False,
                                     transform=test_transform,
                                     download=True)
...
def test(test_loader,epoch, model):
    ...
    for i,(input,target) in enumerate(test_loader):
        ...
        top1.update(acc1[0].item(), input.size(0))
        top5.update(acc5[0].item(), input.size(0))
    f = open('/root/volume/MidProject/cutout_stripe/stripe_'+'final15'+'.txt','a')
...
for epoch in range(epochs):
    ...
    if best_acc < test_acc:
        best_acc = test_acc
        torch.save(model.state_dict(), '/root/volume/MidProject/cutout_stripe/model_updown_'+str(n_holes)+'final15'+'_best.pt')
torch.save(model.state_dict(),'/root/volume/MidProject/cutout_stripe/model_updown'+str(n_holes)+'final1'+'_latest.pt')
